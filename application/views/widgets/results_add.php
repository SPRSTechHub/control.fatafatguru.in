                ...
                <div class="expanel-body">
                    <form action="#" method="POST" name="game_res_frm" id="game_res_frm">
                        <div class="input-group mb-2">
                            ...
                            <input class="form-control" placeholder="Game id" type="text" name="match_id" id="gm_id" />
                            <span class="input-group-text" id="basic-addon2">F</span>
                        </div>
                        <div class="input-group mb-2">
                            <span class="input-group-text bg-dark">BET TYPE</span>
                            <select name="win_type" id="win_type" class="form-control form-select select2"
                        ...
                        <div class="form-group">
                            <div class="input-group input-group-lg">
                                ...
                                <input type="text" value="" id="win_digit" name="win_digit" class="form-control" />
        ...
        <div class="modal-content">
            <div class="modal-header row">
                ...
                <div class="col-md-6 col-6">
                    <select class="form-control select2 form-select" style="width: 220px;" onchange="getGames();"
                        id="mdl_sel_cat">
                        <option value="">Select catagory here</option>
                        <?
                        $gc = $this->home->find(array('status' => 0), 'tbl_game_catagory');
                        if ($gc) {
                            foreach ($gc->result() as $keyvalue) {
                                echo '<option value="' . $keyvalue->cat_id . '">' . $keyvalue->cat_title . '</option>';
                            }
                        } else {
                            echo '<option value="">No catagory found!</option>';
                        }
                        ?>
                    </select>
                </div>
...
function addcallback(params) {
    ...
    var list = '';
    for (let i of data) {
        list += '<a href="javascript:void(0)" onclick="update_game_id(this);" data="' + i['item_id'] +
            '" class="list-group-item list-group-item-action flex-column align-items-start">';
        list += '<div class="d-flex w-100 justify-content-between"><h5 class="mb-1">' + i['item_name'] + '</h5>'
        list += '<small class="text-muted">' + i['match_time'] + '</small></div></a>'
    }
...
function getGames() {
    var x = document.getElementById("mdl_sel_cat").value;
    const formData = new FormData();
    formData.append('cat_id', x);
    formData.append('action', 'getGamesbyCat');
    if (x != '') {
        sendData(formData, '<?= base_url(); ?>admin/', addcallback);
    }
}
